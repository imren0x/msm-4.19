...
#define MAX_BOOST_VOLT		(0xc)	/* max 8V for 98928 */
#else
#define MAX_BOOST_VOLT		(0x1c)	/* max 10V for 98927 */
#endif
...
static void max989xx_debug_init(struct max98927_priv *max989xx,
...
static ssize_t max98927_info_store(struct device *dev,
...
static ssize_t max98927_info_show(struct device *dev,
...
static ssize_t max98927_QFactor_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	pr_err("%s", __func__);
...
static ssize_t max98927_QFactor_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	...
	afe_dsm_get_calib((uint8_t *) payload);
	QFactor = *(payload + 6);
	pr_info("%s: [QFactor] = %d \n", __func__, QFactor);	/* Q29  float value = QFactor / 2^29 */
	ret = snprintf(buf, PAGE_SIZE, "%d\n", QFactor);

	return ret;
}

static ssize_t max98927_temperature_store(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	pr_err("%s", __func__);
...
static ssize_t max98927_temperature_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	...
	afe_dsm_get_calib((uint8_t *) payload);
	coiltemp = *(payload + 4);
	pr_info("%s: [coiltemp] = %d \n", __func__, coiltemp);	/* Q19  float value = QFactor / 2^19 */
	ret = snprintf(buf, PAGE_SIZE, "%d\n", coiltemp);
...
static ssize_t max98927_f0_store(struct device *dev,
...
static ssize_t max98927_f0_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	pr_info("%s: [f0] = %d \n", __func__, f0);	/* Q9  float value = QFactor / 2^9 */
	ret = snprintf(buf, PAGE_SIZE, "%d\n", f0);
...
static ssize_t max98927_impedance_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	pr_err("%s", __func__);
...
static ssize_t max98927_impedance_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
...
static ssize_t max98927_calibrate_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	pr_err("%s", __func__);
...
static ssize_t max98927_calibrate_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
...
__ATTR(temperature, 0444, max98927_temperature_show,
       max98927_temperature_store);
...
static void max989xx_sys_init(struct i2c_client *i2c)
{
	...
	if (ret < 0)
		pr_err("%s sysfs_create_file max98927_calibrate_attr err.",
		       __func__);

	ret = sysfs_create_file(&i2c->dev.kobj, &max98927_impedance_attr.attr);
	if (ret < 0)
		pr_err("%s sysfs_create_file max98927_impedance_attr err.",
		       __func__);
	...
	ret =
	    sysfs_create_file(&i2c->dev.kobj, &max98927_temperature_attr.attr);
	if (ret < 0)
		pr_err("%s sysfs_create_file max98927_temperature_attr err.",
		       __func__);

	ret = sysfs_create_file(&i2c->dev.kobj, &max98927_QFactor_attr.attr);
	if (ret < 0)
		pr_err("%s sysfs_create_file max98927_QFactor_attr err.",
		       __func__);

	ret = sysfs_create_file(&i2c->dev.kobj, &max98927_info_attr.attr);
	if (ret < 0)
		pr_err("%s sysfs_create_file max98927_info_attr err.",
		       __func__);
	return;
}
			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	if (max98927->master) {
		...
		max98927_wrap_update_bits(max98927, MAX98927_PCM_Master_Mode,
					  ...
					  i <<
					  MAX98927_PCM_Master_Mode_PCM_MCLK_RATE_SHIFT);
	}
				  ...
				  struct snd_soc_dai *dai)
{
	...
	pr_info("%s: format supported %d", __func__, max98927->ch_size);
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (mute) {
			...
			max98927_wrap_update_bits(max98927,
						  ...
						  MAX98927_Global_Enable_EN, 0);
			max98927_wrap_update_bits(max98927,
						  MAX98927_AMP_enables,
						  MAX98927_AMP_enables, 0);
			max98927->spk_mode = 0;
			pr_info("%s ------ disable max98927 playback \n",
				__func__);
		} else {
#if 0
			/* we hard code to total gain in hardware pa side, becuase we always put limiter in DSM algorithm side. */
			if (max98927->path_status) {	/* if DSM enable, boost voltage is 10v, total gain = 13dB, not control by mixer ctl from xml */
				max98927_wrap_update_bits(max98927,
				...
				max98927_wrap_update_bits(max98927,
							  ...
							  56);
				max98927_wrap_update_bits(max98927, MAX98927_Boost_Control_0, MAX98927_Boost_Control_0_BST_VOUT_Mask, MAX_BOOST_VOLT);	/* max boost (98927: 10v 98928: 8V) */
			} else {	/* if DSM bypass , reduce gain and boost voltage is 6.5V */
				max98927_wrap_update_bits(max98927,
							  MAX98927_Speaker_Gain,
							  MAX98927_Speaker_Gain_SPK_PCM_GAIN_Mask,
							  1);
			...
			if (max98927->mono_stereo == 0x3) {	/* in stereo mode , we need put some changes into mixer ctl */
				if (max98927->spk_mode & (0x1 << MAX98927L)) {
					regmap_update_bits(max98927->
							   regmap[MAX98927L],
							   MAX98927_AMP_enables,
							   MAX98927_AMP_enables_SPK_EN,
							   1);
					regmap_update_bits(max98927->
							   regmap[MAX98927L],
							   MAX98927_Global_Enable,
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH0) {
		regmap_update_bits(max98927->regmap[MAX98927L],
				   ...
				   sel);
		regmap_update_bits(max98927->regmap[MAX98927L],
				   MAX98927_PCM_Rx_Enables_A, 0xf, sel + 1);
		pr_info("%s: register 0x%02X, value 0x%02X\n", __func__,
					    ...
					    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH0) {
		regmap_read(max98927->regmap[MAX98927L], MAX98927_Global_Enable,
			    &data_global);
		regmap_read(max98927->regmap[MAX98927L], MAX98927_AMP_enables,
			    &data_amp);
		ucontrol->value.integer.value[0] =
					     ...
					     *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH1) {
		regmap_read(max98927->regmap[MAX98927R], MAX98927_Global_Enable,
			    &data_global);
		regmap_read(max98927->regmap[MAX98927R], MAX98927_AMP_enables,
			    &data_amp);
		ucontrol->value.integer.value[0] =
		    (data_global & MAX98927_Global_Enable_EN)
		    & (data_amp & MAX98927_AMP_enables_SPK_EN);
...
static const char *const max98927_boost_current_limit_text[] = {
	"1.0A", "1.1A", "1.2A", "1.3A", "1.4A", "1.5A", "1.6A", "1.7A", "1.8A",
	    "1.9A",
	"2.0A", "2.1A", "2.2A", "2.3A", "2.4A", "2.5A", "2.6A", "2.7A", "2.8A",
	    "2.9A",
	"3.0A", "3.1A", "3.2A", "3.3A", "3.4A", "3.5A", "3.6A", "3.7A", "3.8A",
	    "3.9A",
	"4.0A", "4.1A"
};
...
static const struct snd_kcontrol_new max98927_snd_controls[] = {
	...
	SOC_SINGLE_EXT("Level1 Current Limit",
		       MAX98927_Brownout__level_1_current_limit,
		       0, 63, 0, max98927_reg_get_w, max98927_reg_put_w),
	/* 0 ~ 63 */
	SOC_SINGLE_EXT("Level2 Current Limit",
		       MAX98927_Brownout__level_2_current_limit,
		       0, 63, 0, max98927_reg_get_w, max98927_reg_put_w),
	/* 0 ~ 63 */
	SOC_SINGLE_EXT("Level3 Current Limit",
		       MAX98927_Brownout__level_3_current_limit,
		       0, 63, 0, max98927_reg_get_w, max98927_reg_put_w),
	/* 0 ~ 63 */
	SOC_SINGLE_EXT("Level4 Current Limit",
		       MAX98927_Brownout__level_4_current_limit,
		       0, 63, 0, max98927_reg_get_w, max98927_reg_put_w),
	/* 0 ~ 63 */
			  ...
			  struct max98927_priv *max98927)
{
	...
	max98927->reset_gpio_l =
	    of_get_named_gpio(i2c->dev.of_node, "maxim,98927-reset-gpio", 0);
	...
	if (max98927->reset_gpio_l > 0) {
		...
		if (ret) {
			pr_err
			    ("max98927_i2c_probe : failed to request rest gpio %d error:%d\n",
...
//        max_codec_np =np; */
	...
	max98927->adsp_mode = 0;	/* full mode (RX: playback, TX: feedback_ */
#if 0
	...
	if (IS_ERR(max98927->max989xx_vdd)) {
		...
		if (regulator_count_voltages(max98927->max989xx_vdd) > 0) {
			ret =
			    regulator_set_voltage(max98927->max989xx_vdd,
						  1800000, 1800000);
			if (ret) {
			...
			ret =
			    regulator_set_load(max98927->max989xx_vdd, 200000);
			if (ret) {
	...
	if (IS_ERR(max98927->regmap[id->driver_data])) {
		...
		if (check_max98927_presence(max98927->regmap[id->driver_data])) {
			...
			if (max98927->mono_stereo == 0) {
				...
				for (i = 0;
				     i <
				     sizeof(reg_channel_map[0]) /
				     sizeof(reg_channel_map[0][0]); i++)
					regmap_write(max98927->
						     regmap[id->driver_data],
						     reg_channel_map[0][i][0],
						     reg_channel_map[0][i][1]);
			} else if (max98927->mono_stereo & presence) {
				i2c_states |= presence;	/* mark this chip, then app can address it. */
				for (i = 0;
				     i <
				     sizeof(reg_channel_map
					    [id->driver_data +
					     1]) /
				     sizeof(reg_channel_map[id->driver_data + 1]
					    [0]); i++)
					regmap_write(max98927->
						     regmap[id->driver_data],
						     reg_channel_map[id->
			...
			for (i = 0;
			     i <
			     sizeof(reg_common_map) / sizeof(reg_common_map[0]);
			     i++)
	...
	if (presence) {
		if (max98927->dev == NULL) {
			...
			ret =
			    snd_soc_register_component(&i2c->dev,
			...
			if (ret < 0) {
				pr_err
				    ("max98927 Failed to register component: %d\n",
				     ret);
				i2c_states = 0;
				return ret;
			}
